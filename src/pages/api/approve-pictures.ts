...
import {
  ...
  export default async function handler(
    ...
    try {
      ...
      const url = talent_data.fields?.['Pictures'] || "";
  
      const approval_pictures = talent_data.fields?.['Pictures to Approve']?.split(',') || [];
  
      const pictures = approval_pictures?.filter((item : string, index : number) => item !== req.body.url);
      ...
      const result = await fetch(
        `https://api.airtable.com/v0/${process.env.NEXT_PUBLIC_AIRTABLE_MG_BASE_ID}/${process.env.NEXT_PUBLIC_AIRTABLE_MG_BASE_ACTIVE_TALENT_TABLE_ID}/${req.body.id}`,
        {
          ...
          body: JSON.stringify({
            fields: {
              Pictures: url + req.body.url + ',',
              "Pictures to Approve" : pictures?.join(',')
            },
          }),
        }
      );
