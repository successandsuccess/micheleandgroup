...
export default async function handler(
  ...
  temp_available.push(req.body.talent_id);
    ...
    await insertLog(`<a href="${process.env.NEXT_PUBLIC_SERVER_URI}/dashboard/talents/${req.body.talent_id}" target="_blank">${talent_data.fields["TALENT FULL NAME"]}</a> accepted a job event invite for <a href="${process.env.NEXT_PUBLIC_SERVER_URI}/job-board/${job_data.id}" target="_blank">${job_data.fields['JOB TITLE']}</a`);
