...
export default function StepTwo() {
  ...
  useEffect(() => {
    (async () => {
      if (airtable_id && pageSize) {
        setDataLoading(true);
        setTalentInvited([...talentInvited])
  ...
  const handleAllAddTalent = () => {
    ...
    for (let i = 0; i < records?.length; i++) {
      const isIdInArray = talentInvited.some(
        (item: any) => item.id === records[i].id
      );
      if (!isIdInArray) {
        pendinglist.push({
          id: records[i].id,
            ...
            <>
              <ModalBody>
                <div className="w-full py-3 ">
                  <p className="font-bold">Want to add this talent to job?</p>
                  <div className="flex flex-col gap-[10px]">
                    {/* Section1 */}

                    <div className="bg-white rounded-3xl p-3 flex flex-col mx-auto  w-[230px]">
                      <img
                        className="rounded-3xl w-[230px] h-[230px] object-cover object-top"
                        src={
                          data?.Pictures
                            ? data["Pictures"]?.split(",")?.[0]
                    ...
                    <div className="flex flex-row justify-between">
                      ...
                      <Button color="primary">Add talent to this job</Button>
                    </div>
                  </div>
                </div>
              </ModalBody>
